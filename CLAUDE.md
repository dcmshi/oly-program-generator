...
- **HTML/web paragraph extraction** — same bug class as EPUB: `html_extractor.block_text()` applies the identical `NavigableString('\n\n')` fix and is used by both `extract_text_from_html()` and `ingest_web.fetch_article()`. Never call `get_text(separator='\n')` on content headed for the chunker. NOTE: the 418 Catalyst articles were originally ingested pre-fix (446 chunks ≈ 1/article) — re-ingest pending on the main DB machine (see task list #6).
...
- **`_llm_classify()` fires when heuristic confidence < 0.6** — short sections (<50 words) score 0.60 (just at threshold), so they won't trigger LLM. Signal-free narrative scores a confident 0.80 (no paid call on plain prose). Only genuinely AMBIGUOUS mid-length sections — a weak/partial signal (a lone principle indicator, or stray percentages without principle language) that cleared no category — score 0.55 and fall through to the LLM (audit5-M2: before this the default was a flat 0.80 and the fallback was dead code).
...
- **Pipeline / web separation** — the 6-step agent pipeline (`orchestrator.py` and its imports) has zero imports from `web/`. It is usable as a standalone library or CLI without FastAPI installed. The only coupling point is `web/worker.py` which does `import orchestrator` to run the pipeline as an ARQ background task. Keep this unidirectional: pipeline modules must never import from `web/`.
