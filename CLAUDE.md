...
- **Prompt length budget:** worst case is ~14,000 chars against `PROMPT_LENGTH_WARN_CHARS = 20,000`. The two dominant sections are Available Exercises (~78 chars each) and `MAX_CONTEXT_CHUNKS` × `SNIPPET_MAX_CHARS` (4 × 1,500) knowledge chunks. If the exercise catalogue passes ~100 entries, add `MAX_EXERCISES_IN_PROMPT` to `shared/constants.py` and slice `retrieval_context.available_exercises` at the `ex_lines` loop in `generate.py`.
