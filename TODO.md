...
- [x] **MEDVEDEV — 613 session-block chunks of ~324 chars.** ✅ code 2026-09-20. **Dev-copy re-ingest of source 501 is incomplete — the API credit balance ran out at the principle batch (run #1127 failed, checkpoint rewound; 388 chunks landed with the 0.4 flush fraction and only 78 got a `context_prefix`).** After a top-up: delete source 501's chunks and re-run the same command (`--vision --batch --contextualize`, ≈ $1.50 OCR + ≈ $1 template parses; OCR is not cached — ING-M5). The two runs also surfaced and fixed: OCR page groups truncated at 8,192 tokens now regrow to 16k in both paths (the batch path re-sent 2 groups in a round-2 batch); batch `custom_id`s may only hold `[A-Za-z0-9_-]` (a colon 400'd the first principle batch — now validated before submission); the flush fraction went 0.4 → 0.7 because Medvedev's other half (exercise-selection listings, ~150 tokens per week) came out as 1–2-week chunks of ~300 chars — replayed: listings median 1,305 chars, logs 4–5 sessions per chunk. Also: the run added 14 joined-section templates next to the 12 page-fragment ones from March (36 total) — dedupe like TAKANO-TEMPLATES after the re-run. Root cause was upstream of the chunker: vision OCR renders the book's session labels as `# 4 - February (Monday)`, which `classifier._split_into_sections`' markdown-heading regex `#{1,3}\s+.+` swallowed — every training session became its own section (run 530: 631 sections → 613 chunks; the label went into `title`, hence the empty `section` column), and the week markers (`Totals for week 8 … Week # 9`) hung off the previous session's tail. Fix: the heading regex excludes `# <digits> -`; `SemanticChunker._chunk_section` starts a paragraph at any `Week #? N` line and closes the chunk there once it holds ≥ `WEEK_BOUNDARY_FLUSH_FRACTION` (0.4) of the profile size, with no tail overlap across the boundary. Measured on three real OCR'd pages: 1 section instead of 12, chunks of 1,151–1,954 chars / 343–645 tokens holding 3–5 sessions each and starting at the week label (a week is 350–650 tokens, so the 700-token `soviet` profile already fits one — size unchanged). Tests `test_ocr_session_labels_are_not_markdown_headings`, `test_training_log_chunks_per_week`. Runbook §9 + CLAUDE.md invariant. **Follow-up:** 50 graded ids in `eval/golden.json` (28 relevant) pointed at the old Medvedev chunks — the golden rebuild after RELABEL covers it.
...
- [ ] **MODEL-2 — open-weight / third-party providers via OpenRouter** (plan in §11: Fireworks refused a Canadian signup 2026-09-20; OpenRouter has no markup beyond 5.5 % on credits and lists GLM-5.3-Flash at $0.09 / $0.30 and DeepSeek V4.1 Flash at $0.15 / $0.60 — 10–25× under Sonnet 5 batch; `openai_compat` provider per model role behind `shared/llm.py`, provider-order pinning, `model_baseline.py --config openrouter:<id>`; STRUCT-1 first or alongside; light-model role is the first production switch; Kimi K3 out).
