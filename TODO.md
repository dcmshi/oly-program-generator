...
- [ ] **JEV-1 — Jev (TypeSafe AI) for the corpus's judgement calls.** 2026-09-20: `processors/jev_judge.py` + `relabel_chunk_types.py --judge jev` landed and ran live (60-chunk probe: 185 ms/decision, $0.0025, mean confidence 0.82, 75 % agreement with the Haiku labels — the disagreements are junk passages at 0.35–0.5 confidence, below the write threshold, and a few high-confidence corrections that read as right; 200-chunk dry run: 45 would change). Not applied to the corpus yet — it would re-freeze the golden set/baseline frozen on the Haiku labels today. Next fits, in order: (1) apply `--judge jev` corpus-wide and rebuild golden/baseline in the same pass; (2) the section classifier (`_llm_classify`, a `Choice` over 6 types — could replace the heuristic-then-LLM two-pass entirely); (3) PRIN-DEDUPE via `Noul("same rule?")` over name-similar principle pairs; (4) the golden-set grader as a `Score` 0/1/2 (judge change → new baseline); (5) a `Score` reranker over the top-20 hybrid candidates per session query (~$0.005 per program), gated by `run_eval`. Not for generation, extraction, parsing, OCR or contextualize. $5 of starter credit covers all of it many times over.
