...
- [x] ✅ *Done 2026-09-22 as EMBED-1 (`TODO.md`, `docs/RETRIEVAL_EVAL.md`): nDCG@5 0.664 → 0.721.* **#22 — Embedding upgrade: `text-embedding-3-large` @ dim 1536** (PREREQ #20/#21). Matryoshka truncation → keep `vector(1536)`, no schema change: pass `dimensions=1536` in `_embed_batch`/`_embed`, flip `config.py:33`. Backfill ~50-line script from stored `content` (~3.4M tokens ≈ **$0.45**); HNSW updates in place; re-run eval.
